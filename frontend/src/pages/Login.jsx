...
function Login() {
  ...
  const onsubmitHandler = async(event)=>{
    try {
      ...
      else if(currentState === "Login" && email && password){
  ...
  return (
    <form onSubmit={onsubmitHandler} className='flex flex-col items-center w-[90%] sm:max-w-96 m-auto mt-14 gap-4 text-gray-800'>
      ...
      <input onChange={(e)=>setPassword(e.target.value)} value={password} className='w-full px-3 py-2 border border-gray-800' type="password" placeholder='password' required min={8}/>
      <div className='flex w-full justify-between text-sm -mt-2'>
        <p>Foregot your password?</p>
        {
          currentState === "Login"
          ? <p onClick={(e)=>setCurrentState('Sign Up')} className='cursor-pointer'>Create account</p>
          : <p onClick={(e)=> setCurrentState('Login')} className='cursor-pointer'>Sign In</p>
        }
      </div>
      <button type='submit' className='bg-black cursor-pointer text-white font-light px-8 py-2 mt-4'>{currentState === 'Login' ? "Sing In" : "Sign Up"}</button>
    </form>
  )
}
